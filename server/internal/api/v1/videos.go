...
func registerVideoRoutes(g *gin.RouterGroup, deps api.Deps) {
	...
	g.POST("/videos", middleware.RequireAuth(), func(c *gin.Context) {
	...
	g.GET("/videos/:id/stream", func(c *gin.Context) {
		id, err := uuid.Parse(c.Param("id"))
		if err != nil {
			handlerutil.Error(c, http.StatusBadRequest, apperror.ValidationError, "入力内容を確認してください", []apperror.FieldDetail{
				{Field: "id", Message: "UUID形式で指定してください"},
	...
	g.PATCH("/videos/:id", middleware.RequireAuth(), func(c *gin.Context) {
		id, err := uuid.Parse(c.Param("id"))
		if err != nil {
			handlerutil.Error(c, http.StatusBadRequest, apperror.ValidationError, "入力内容を確認してください", []apperror.FieldDetail{
				{Field: "id", Message: "UUID形式で指定してください"},
	...
	g.DELETE("/videos/:id", middleware.RequireAuth(), func(c *gin.Context) {
		id, err := uuid.Parse(c.Param("id"))
		if err != nil {
			handlerutil.Error(c, http.StatusBadRequest, apperror.ValidationError, "入力内容を確認してください", []apperror.FieldDetail{
				{Field: "id", Message: "UUID形式で指定してください"},
	...
	g.GET("/me/videos", middleware.RequireAuth(), func(c *gin.Context) {
		uid, ok := middleware.UserID(c)
		if !ok {
			handlerutil.Error(c, http.StatusUnauthorized, apperror.Unauthenticated, "ログインが必要です", nil)
			return
